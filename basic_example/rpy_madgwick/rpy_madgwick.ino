...
float read_data(float gyro_raw[3], float gyro_data[3], float acc_raw[3], float acc_data[3]){
...
void calc_offset(float gyro_offset[3], float acc_offset[3]){
  float gyro_raw[3];
  float gyro_data[3];
  float acc_raw[3];
  float acc_data[3];
  ...
  delay(100);
  for(int i = 0;i < 50;i++){
    read_data(gyro_raw, gyro_data, acc_raw, acc_data);
    for(int j = 0;j < 3;j++){
...
void setup(){
  ...
  reset_imu();
  delay(1000);
  calc_offset(gyro_offset, acc_offset);
  set_gyroRange(0);
  set_gyroLPF(6);
  set_accRange(0);
  set_accLPF(6);
...
void loop(){
  ...
  float acc_data[3];
  read_data(gyro_raw, gyro_data, acc_raw, acc_data);
  MadgwickFilter.updateIMU(gyro_data[0], gyro_data[1], gyro_data[2], acc_data[0], acc_data[1], acc_data[2]);
  roll = MadgwickFilter.getRoll();
  pitch = MadgwickFilter.getPitch();
  yaw  = MadgwickFilter.getYaw();
  Serial.println("%%%%%%%%%%%%%%%Roll Pitch Yaw data with Madgwick filter%%%%%%%%%%%%%%%");
  Serial.print("Roll: ");
  Serial.print(roll);
  Serial.println("°");
  Serial.print("Pitch: ");
